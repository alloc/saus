...
export function createHotReload(
  ...
  const { server, resolveId, events, logger } = context
  ...
  async function reloadAffectedFiles() {
    ...
    console.log('currentReload has %O dirty files', dirtyFiles.size)
    ...
    console.log('currentReload completed!')
  ...
  async function reloadFile(file: string) {
    const getPendingReload = () => {
      if (!pendingReload) {
        console.log('pendingReload set by %O', file)
      }
